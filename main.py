...
# 核心物理校准常数
# 芯片在当前积分时间/增益下的典型暗电流计数（可通过遮挡传感器闭灯测量得到）
# 扣除暗计数能显著提升光谱重建在低基准电压下的线性度
DARK_FLOOR = 12.0  
...
try:
    ...
    while True:
        ...
        if sensor.is_data_ready():
            ...
            # B. 物理量纲预处理
            # 严格按照从 CSV 读取到的通道顺序提取，并扣除暗电平基底，保证进入矩阵的光强信号满足齐次性
            X_input = []
            for ch in matrix_channels:
                net_val = float(spectrum[ch]) - DARK_FLOOR
                if net_val < 0.0:
                    net_val = 0.0
                X_input.append(net_val)

            # C. 执行实时光谱重建计算 (一维矩阵点乘与非负物理截断)
